...
namespace Csharp_Project
{
    public partial class FORM_NEW_USER : Form
    {
        ...
        {
            if(TB_USERNAME.Text == string.Empty || TB_FULLNAME.Text == string.Empty || TB_PASSWORD.Text == string.Empty || TB_TEL.Text == string.Empty || TB_EMAIL.Text == string.Empty || cmbRole.SelectedItem.ToString() == string.Empty)
            {
                MessageBox.Show("One Or More Fields Are Empty", "Empty Fields", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else
            {
                user.insertUser(TB_USERNAME.Text, TB_FULLNAME.Text, TB_PASSWORD.Text, TB_TEL.Text, TB_EMAIL.Text, cmbRole.SelectedItem.ToString());
